...
/**
 * Staged, committable PHP files (excludes vendor and tests).
 *
 * @return string[]
 */
...
foreach ( $files as $file ) {
	...
	foreach ( $lines as $i => $line ) {
		if ( preg_match( '/function\s+\w+\s*\(([^)]*)\)/', $line, $m ) ) {
			$ln     = $i + 1;
			$params = trim( $m[1] ) === '' ? array() : explode( ',', $m[1] );
 ...
 *
 * @param string $src PHP source to tokenize.
 * @return array<int,true>
 */
function php_control_flow_lines( string $src ): array {
	$targets = array( T_IF, T_FOREACH, T_FOR, T_WHILE, T_SWITCH );
	$lines   = array();
 ...
 * @param int      $start Line index of the function signature.
 * @return int
 */
